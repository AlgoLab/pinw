...
function legend_box() {
  ...
  var s_l = d3.select("#legend-container").append("svg:svg")
    .attr("id", "legend")
    .attr("width", l_w)
    .attr("height", l_h);
    //.style("position", p_s.pos)
    //.style("left", p_s.left)
...
function svg_expande_box() {
  ...
  var s_i = d3.select('#expande-container').append('svg:svg')
    .attr('id', 'expande_box')
    .attr('width', s_w)
    .attr('height', s_h);
...
function connect_exon(exon, sequence) {
  ...
  point = [
    { "source" : { "x" : xr_s + margin_isoform.left, "y" : yr_s},
