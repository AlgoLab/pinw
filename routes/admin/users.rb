...
class PinW < Sinatra::Application
    ...
    post '/admin/users/edit-password' do
      user = User.find(params[:user_id])

      if not user.authenticate(params[:InputOldPassword])
        redirect to "/admin/users/edit/#{params[:user_id]}?err=2"
      end

      user.password =  params[:InputNewPassword]
      ...
      UserHistory.create( :admin_id => session[:user].id, :subject_id => user.id, :message => "Password changed" )
      redirect to '/admin/users?ok=1'
    end
